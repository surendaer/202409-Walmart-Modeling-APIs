...
def update(
    ...
    price: float | None = None,
    count: int | None = None,
    category: Category | None = None,  # Added category parameter
) -> dict[str, Item]:

    if item_id not in items:
        raise HTTPException(status_code=404, detail=f"Item with {item_id=} does not exist.")
    if all(info is None for info in (name, price, count, category)):  # Added category to the check
        raise HTTPException(
    ...
    if count is not None:
        item.count = count
    if category is not None:  # Added category update
